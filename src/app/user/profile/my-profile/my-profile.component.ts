...
export class MyProfileComponent implements OnInit {
  ...
  getDays() {
    this.service1.getApi('/admin/getCLubTimings').subscribe({
      next: (resp) => {
        this.data = resp.schedule;
        this.prepareTimes();
      },
      error: (error) => {
        console.log(error.message);
  ...
  times: { day: string, timeStart: string, timeEnd: string, isClosed: any }[] = [];

  prepareTimes(): void {
    this.times = this.data.map((item: { id: any, day: any; timeStart: any; timeEnd: any; isClosed: any; }) => ({
      id: item.id,
      day: item.day,
      timeStart: this.formatTime(item.timeStart),
      timeEnd: this.formatTime(item.timeEnd),
  ...
  formatTime(time: string | null): string {
    if (!time) {
      return '00:00'; // Default if null or empty
    }
  ...
  saveChanges(item: any): void {
    ...
    if (item.isClosed) {
      formURlData.set('scheduleId', item.id);
      formURlData.set('isClosed', 1);
    } else {
      formURlData.set('scheduleId', item.id);
      formURlData.set('isClosed', 0);
      formURlData.set('timeStart', item.timeStart);
      formURlData.set('timeEnd', item.timeEnd);
    }



    // this.service1.postAPI('/admin/updateClubTiming', this.formdata).subscribe({
    this.service1.postAPI('/admin/updateClubTiming', formURlData).subscribe({
      next: (resp) => {
        if (resp.success === true) {
          this.toastr.success(resp.message);
        } else {
