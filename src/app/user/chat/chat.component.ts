...
export class ChatComponent {
  ...
  sendMessage() {
    ...
    messagesCollection.add(messageModel)
      .then(() => {
        ...
        this.sendChatNotification(this.chatId, messageContent)
  ...
  sendChatNotification(userId: string, messageModel: any) {
    const formURlData = new URLSearchParams();
    formURlData.set('content', messageModel);
    formURlData.set('user_id', userId);
    this.userSer.postAPI('/admin/sendChatNotification', formURlData.toString()).subscribe({
      next: (resp) => {
